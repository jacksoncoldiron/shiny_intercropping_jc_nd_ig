...
crop2_top10 <- crop_1_intercrop |>
  group_by(crop2)|>
  summarize(n = n())|>
  arrange(desc(n))|>
  slice(1:10)

ggplot(crop_1_intercrop |> filter(crop2 %in% crop2_top10$crop2)|> drop_na(ler_crop1, ler_crop2), aes(x = end_year, color = crop2))+
  line()+
  theme_bw()+
...
intercrop_time_crops <- intercrop_LER |>
...
cumulative_exps_by_crop <- intercrop_time_crops |>
...
cumulative_exps_by_crop2 <- intercrop_time_crops |>
  group_by(crop1, crop2) |>
  mutate(cumulative_experiments = cumsum(count), 
